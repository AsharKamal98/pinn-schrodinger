...
class tdSchrodinger_iw(torch.nn.Module):
    def __init__(self, 
                 device, 
                 num_x_points, 
                 num_t_points,
        ...
        self.u_bc = torch.tensor([[u_boundary[0]], [u_boundary[1]]]).to(device).repeat(num_t_points, 1)  # shape (2*num_t_points, 1)

        # initial conditions for loss
        ...
        # compute E0 from analytic psi0(x) = sqrt(2) sin(nπx), V=0
        x0 = self.x_t_init[:, 0:1]
        psi0_re = torch.sqrt(torch.tensor(2.0, device=self.device)) * torch.sin(self.n * math.pi * x0)
        dpsi0_dx = torch.sqrt(torch.tensor(2.0, device=self.device)) * self.n * math.pi * torch.cos(self.n * math.pi * x0)
    ...
    def loss(self, x):
        ...
        cnt_per_t = torch.zeros_like(uniq_t).scatter_add(0, inv, torch.ones_like(prob))  # #x per t
        mean_prob_per_t = sum_per_t / cnt_per_t   
    ...
    def make_plot(self, u_pred):
        ...
        line_true, = ax.plot([], [], lw=2, ls="--", label="True |ψ|²")  # dashed line

        title_template = f"State={self.n}   |   t={{t:.2f}}"
        ax.set_xlim(x_vals.min(), x_vals.max())
        # ymax = max(prob_pred.max(), prob_true.max()) * 1.1
        y_max = max(prob_pred.max(), np.abs(u_pred_real).max(), np.abs(u_pred_imag).max(), prob_true.max()) * 1.1
        #ax.set_ylim(0, ymax)
        #ax.set_xlim(-3,3)
        ...
        def update(frame):
            ...
            return line_prob, line_real, line_imag, line_true, title
